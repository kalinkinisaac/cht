...
class TestGetExpiredTables:
    ...
    def test_mixed_tables(self):
        ...
        assert result.iloc[0]["expired"] == True  # Use == for pandas comparison
        assert result.iloc[1]["table"] == "active_table"
        assert result.iloc[1]["expired"] == False
        assert result.iloc[2]["table"] == "no_expiry"
        assert result.iloc[2]["expired"] == False
...
class TestIntegrationWithCluster:
    """Test integration with Cluster class methods."""
    
    def test_cluster_cleanup_temp_tables(self):
        """Test Cluster.cleanup_temp_tables method."""
        from cht.cluster import Cluster
        
        cluster = Cluster("test", "localhost")
        cluster.query_raw = MagicMock()
        cluster.query = MagicMock()
        
        mock_result = MagicMock()
        mock_result.result_rows = []
        cluster.query_raw.return_value = mock_result
        
        result = cluster.cleanup_temp_tables("temp", dry_run=True)
        assert result["database"] == "temp"
        assert result["dry_run"] is True
    
    def test_cluster_get_temp_tables(self):
        """Test Cluster.get_temp_tables method."""
        from cht.cluster import Cluster
        ...
        result = cluster.get_temp_tables("temp")
        assert isinstance(result, pd.DataFrame)
    
    def test_cluster_create_temp_table(self):
        """Test Cluster.create_temp_table method."""
        from cht.cluster import Cluster
        ...
        assert table_name == "temp.test_table"
        assert cluster.query.call_count == 2  # CREATE + ALTER